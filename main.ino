...
#define MUX_S0 D1  // Pin S0 of 74HC4051 connected to NodeMCU D1
#define MUX_S1 D2  // Pin S1 of 74HC4051 connected to NodeMCU D2
#define MUX_S2 D5  // Pin S2 of 74HC4051 connected to NodeMCU D5
#define MUX_SIG A0 // Analog pin connected to SIG pin of 74HC4051

// Define sensor pins
#define WATER_PUMP_PIN D7 // Water Pump
...
float calibrationSlope = 1.6512113655911938;        // Calculated pH calibration slope
float calibrationIntercept = 0.0014312808810704651; // Calculated pH calibration intercept
...
void setup()
{
    ...
    digitalWrite(WATER_PUMP_PIN, LOW); // Ensure water pump is off initially
...
void MainFunction()
{
    ...
    {
        Blynk.logEvent("soil_alert", "Attention: Water content is too low. Please water the plants.");
        Serial.println("Soil Moisture Content is Low!!!");

        // Check for pH Levels
        if (pHSensorValue < pHThresholdLow)
        {
            Blynk.logEvent("phAcidic", "Alert: Water pH is below 6. Possible acidity detected. Please consider changing the water.");
        ...
        {
            Blynk.logEvent("pHBasic", "Alert: Water pH Level High! Possible basic content detected. Check water quality.");
        ...
        {
            Serial.println("pH is normal so watering the plants");
            while (soilMoistureSensorValue <= soilThresholdValue)
            {
                digitalWrite(WATER_PUMP_PIN, HIGH);
                soilMoistureSensorValue = calculateSoilMoistureValue();
            }
        }
    }
    delay(1000);
...
float calculateSoilMoistureValue()
{
    ...
    // Map the moisture sensor value to a percentage (assuming 0-1023 range)
...
float calculatepHSensorValue()
{
    ...
    // Convert pH sensor value to actual pH value using calibration parameters
